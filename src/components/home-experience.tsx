...
export function HomeExperience() {
  ...
  return (
    <main className={`motion-page ${darkMode ? 'site-shell dark-mode' : 'site-shell'}${themeSwitching ? ' theme-switching' : ''}`}>
      ...
      <section id="collections" aria-labelledby="collections-title" className="mx-auto flex max-w-7xl flex-col gap-10 px-5 py-16 lg:px-8 lg:py-24"><div className="flex items-end justify-between gap-6"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-primary">Collection</p><h2 id="collections-title" className="mt-3 font-mono text-4xl font-black uppercase tracking-[-0.05em] sm:text-6xl">The current drop</h2></div><span className="font-mono text-sm font-bold text-muted-foreground">SS24</span></div><div id="shop" className="grid gap-6 md:grid-cols-3">{products.map((product, index) => <Card key={product.name} className="motion-card group motion-reveal overflow-hidden rounded-xl border-border bg-card py-0 shadow-none" style={{ '--motion-delay': `${index * 90 + 120}ms` } as CSSProperties}><div className="relative bg-muted p-0 overflow-hidden"><div className="absolute bottom-0 left-0 right-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out bg-gradient-to-t from-foreground/80 to-transparent p-4 z-10"><Button className="w-full" onClick={() => addToCart(index)}>Shop now <ArrowRight data-icon="inline-end" /></Button></div><Badge className="absolute left-4 top-4 z-10" variant="secondary">{product.badge}</Badge><Button variant="secondary" size="icon" className="absolute right-4 top-4 z-10 rounded-full" aria-label={`${liked.includes(index) ? 'Remove' : 'Add'} ${product.name} ${liked.includes(index) ? 'from' : 'to'} wishlist`} onClick={() => toggleLiked(index)}><Heart className={liked.includes(index) ? 'fill-primary text-primary' : ''} /></Button><Image src={product.image} alt={`${product.name}, ${product.type}, from the Town Team Egyptian streetwear collection`} width={700} height={850} sizes="(max-width: 768px) 100vw, 33vw" className="motion-image block aspect-[4/5] w-full object-cover" /></div><CardContent className="flex flex-col gap-2 p-5"><p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-primary">{product.type}</p><h3 className="font-mono text-2xl font-bold uppercase tracking-[-0.03em]">{product.name}</h3><p className="text-sm text-muted-foreground">{product.price}</p></CardContent></Card>)}</div></section>
